...
# 实现 Icon 组件
这里需要注意的是，几乎所有现代框架都在把图标方案从`font`变为`svg`，从前文引入的`element-plus`导航中的图标，可以看到，在`plus`版本，图标已经变为了`svg`，这里是要实现一个可全局使用的`svgIcon`组件

### 自定义 svg 图标组件
首先在`src/plugins`文件夹下新建`svgBuilder`文件夹，并在其内创建`index.ts`文件：
```
...
```
此时home页面预览出现关闭按钮，则代表`svg`图标插件已引入完成

### element-plus 图标引入
由于图标方案的改变，目前图标转为`svg`，且变为了按需加载，所以要想使用哪个图标，也必须先引入要使用的那个图标组件，这里可以把所有图标组件在全局注册，引用会更方便一些
```
# src/main.ts
import { createApp } from 'vue'
import router from './router'
...
```
这里使用的工具函数将所有`element-plus`的图标组件在注册为全局组件时，组件名都加了前缀，前缀可自定义，我这里加的前缀是`kevin`

在`home`页面引用`element-plus`的图标
```
<el-icon><kevin-zoom-out /></el-icon>
```
预览页面，能正常显示缩小的图标，则引入正常

**这种引入方式只能作为直接引用的形式使用，因其组件名就是图标名，无法动态赋值，所以只作为补充使用，svgIcon组件更适合多场景使用**

`element-plus`自身组件上使用图标，组件自身有引用，无需额外引入，可在`home`页面做个测试，引入日期选择插件：
```
# src/views/home/index.vue
<script setup lang="ts">
import { ref } from 'vue'
...
```
此时运行项目可发现，日历图标能正常显示，赋值后的清除按钮图标也能正常显示

**此时打开F12查看，会发现控制台有无穷多的警告，这个比较影响开发，其原因是element-plus的版本问题**
